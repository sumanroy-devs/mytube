...
MyTube has no account system, no analytics or telemetry SDK, no crash reporting service, and no
...
Used by one screen: Settings > Device Sync. MyTube can copy a library between two of the user's own
...
2. Recovering downloads. MyTube can save downloads to a public folder that survives an uninstall. On
...
needs no permission. On ROMs where PiP is missing, disabled by the vendor, or broken, MyTube can draw
...
unless the user opts into a custom location in Download settings, at which point MyTube can write to
...
`DOWNLOAD_WITHOUT_NOTIFICATION` is a leftover. MyTube downloads through its own service and does not
...
These are not declared in MyTube's own manifest. They are merged in from dependencies.

`RECEIVE_BOOT_COMPLETED` comes from `androidx.work:work-runtime`. WorkManager uses it to restore
scheduled jobs after a reboot. MyTube's jobs are the subscription check, the upcoming-video reminder,
the optional auto-backup, and, on the `github` flavor, the update check. MyTube registers no boot
receiver of its own.
...
MyTube ships two flavors. The permission difference between them is the one above.
...
## What MyTube does not do

- No account, login, or user identifier of any kind.
- No analytics, telemetry, crash reporting, or advertising SDK.
- No background microphone, camera, or location access. MyTube declares no location permission.
